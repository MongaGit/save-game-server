...
function em_advanced_settings:onWorldmapTickBoxChange(_optionIndex, _value, _settingID)
	em_settings.worldmap_settings[_settingID].enabled = _value;
	em_settings:save();
end
...
function em_advanced_settings:initialise()
	...
	local worldmap_settings = em_settings.worldmap_settings;
	local color_settings = em_settings.color_settings;
	local map_settings = em_settings.map_settings;
	...
	self.label_worldmapOptions = ISLabel:new(x, y, 16, "Change World Map Settings", 1, 1, 1, 1, UIFont.Small, true);
	self.label_worldmapOptions:initialise();
	self.label_worldmapOptions:instantiate();
	self:addChild(self.label_worldmapOptions);

	y = y + 20;

	for settingID, settings in pairs(worldmap_settings) do
		if settingID ~= "settingsToggle" then
			self[settingID] = ISTickBox:new(x, y, 100, 16, "", self, em_advanced_settings.onWorldmapTickBoxChange, settingID);
			self[settingID].choicesColor = {r=1, g=1, b=1, a=1};
			self[settingID]:initialise();
			self[settingID]:instantiate();
			self[settingID].selected[1] = settings.enabled;
