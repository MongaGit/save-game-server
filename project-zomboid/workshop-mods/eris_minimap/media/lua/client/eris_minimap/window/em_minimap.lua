...
local function ignoreEvent() return false; end;
local worldMapMouseWheel;
...
function em_minimap:IWBUMSonMouseWheel(_mod)
	self.map:quickupdateViewPortData();
	self.mapIconMetaGroup:update();
	return false;
end
...
function em_minimap:doGlobalContextOptions()
	...
	if em_core.adminMode or em_core.debugMode then
...
function em_minimap:IWBUMSrender()
	if not self:getIsVisible() then return; end;
	if em_core.window.moving or em_core.window.isCollapsed then return; end;

	self:setStencilRect(0, 0, self:getWidth(), self:getHeight());

	if em_core.WORLDMAP and em_settings.context_settings.showWorldMap.enabled then
		em_settings.map_settings.zoomLevel = math.max(self.worldMap.mapAPI:getWorldScale() * tileRatio, 0.01);
		--self.worldMap:prerender();
		self.worldMap.mapAPI:centerOn(em_core.map.vpCenterXInWorld, em_core.map.vpCenterYInWorld);
		self.worldMap:render();
	end;

	if em_settings.context_settings.showTileMap.enabled then
		self.map:render();
	end;
...
function em_minimap:updateWorldMapSettings()
	if self.worldMap and self.worldMap.mapAPI then
		for optionID, option in pairs(em_settings.worldmap_settings) do
			self.worldMap.mapAPI:setBoolean(optionID, option.enabled);
		end;
	end;
end
...
function em_minimap:update(_updateTime)
	...
	if em_core.WORLDMAP then
		self.worldMap:setWidth(self:getWidth());
		self.worldMap:setHeight(self:getHeight());
		if self.worldMap.mapAPI then
			self.worldMap.mapAPI:centerOn(em_core.map.vpCenterXInWorld, em_core.map.vpCenterYInWorld);
			self.worldMap.mapAPI:setBoolean("Isometric", em_settings.map_settings.viewMode == 2 and true or false);
		end;
		self.worldMap:setVisible(em_settings.context_settings.showWorldMap.enabled);
		if em_settings.context_settings.showWorldMap.enabled then
			self.onMouseWheel = em_minimap.IWBUMSonMouseWheel;
			self.worldMap.onMouseWheel = worldMapMouseWheel;
		else
			self.worldMap.onMouseWheel = ignoreEvent;
			self.onMouseWheel = em_minimap.onMouseWheel;
		end;
		self:updateWorldMapSettings();
		--em_settings.context_settings.transparencyToggle.enabled = false;
	end;
end
...
function em_minimap:initialise()
	...
	if em_core.WORLDMAP then
		self.worldMap = ISMiniMapInner:new(0, 0, self:getWidth() - 10, self:getHeight() - 10, 0);
		self.worldMap:instantiate();

		local dirs = getLotDirectories();
		for i=1,dirs:size() do
			local file = 'media/maps/'..dirs:get(i-1)..'/worldmap.xml';
			if fileExists(file) then
				self.worldMap.mapAPI:addData(file);
			end
			self.worldMap.mapAPI:addImages('media/maps/'..dirs:get(i-1));
		end
		self.worldMap.mapAPI:setBoundsFromWorld();
		self.worldMap.mapAPI:setZoom(19);

		local HideUnvisited = not (em_core.adminMode or getDebug());
		--self.worldMap.mapAPI:setBoolean("HideUnvisited", HideUnvisited);
		--self.worldMap.mapAPI:setBoolean("Players", false);
		--self.worldMap.mapAPI:setBoolean("Symbols", false);
		--self.worldMap.mapAPI:setBoolean("MiniMapSymbols", false);
		--self.worldMap.mapAPI:setBoolean("ImagePyramid", true);

		self.worldMap.onMouseUp = ignoreEvent;
		self.worldMap.onMouseUpOutside = ignoreEvent;
		self.worldMap.onMouseDown = ignoreEvent;
		self.worldMap.onRightMouseUp = ignoreEvent;
		self.worldMap.onRightMouseDown = ignoreEvent;
		self.worldMap.onMouseMoveOutside = ignoreEvent;
		self.worldMap.onMouseMove = ignoreEvent;
		self.worldMap.onMouseMoveOutside = ignoreEvent;

		worldMapMouseWheel = self.worldMap.onMouseWheel;
		--self.worldMap.onMouseWheel = ignoreEvent;
		MapUtils.initDefaultStyleV1(self.worldMap);

		self:addChild(self.worldMap);
		self.worldMap:bringToTop();

		em_minimap.render = em_minimap.IWBUMSrender;
	end;
