...
require("eris_minimap/icons/em_mapIconBase");
...
--if em_core.WORLDMAP then em_mapIconGroupBase.canSee2D = em_mapIconGroupBase.newCanSee2D; end;
...
function em_mapIconGroupBase:canSee2D(_icon1, _icon2, _isOutside)
	...
	if self:deferPreCheck(_icon2) then
		...
		if _icon1.isAtHome then
			...
			local sqObj = _icon2:getObj() and _icon2:getObj():getSquare();
			if sqObj and sqObj:IsOnScreen() and sqObj:isCanSee(_icon1:getObj():getPlayerNum()) then seen = true; end;
		end;
	end;
	self:deferPostCheck(_icon2, seen, precheck);
	return seen;
...
function em_mapIconGroupBase:doIconTransform(_icon, _mapTransform)
	local map = em_core.map;
	local mapTransform = _mapTransform or em_core.map.transforms.mapTransform;
...
function em_mapIconGroupBase:renderIcons()
	local drawTexture = em_core.minimap.drawTextureAllPoint;
	local drawIcons = self.drawIcons;
