[![Docker Pulls](https://img.shields.io/docker/pulls/lorthe/monga-minecraft.svg)](https://hub.docker.com/r/lorthe/monga-minecraft/)
[![Docker Stars](https://img.shields.io/docker/stars/lorthe/monga-minecraft.svg?maxAge=2592000)](https://hub.docker.com/r/lorthe/monga-minecraft/)
...
Esta imagem do docker fornece um Minecraft Server que baixará automaticamente a versão estável mais recente
versão na inicialização. Você também pode executar/atualizar para qualquer versão específica ou o
instantâneo mais recente. Veja o _Versions_ seção abaixo para mais informações.
Para simplesmente usar a última versão estável, run

    docker run -d -it -p 25565:25565 -e EULA=TRUE lorthe/monga-minecraft

onde, neste caso, a porta padrão do servidor 25565 será exposta em sua máquina host.

> Se você planeja executar um servidor por um longo período de tempo, é altamente recomendável usar uma camada de gerenciamento como [Docker Compose](#using-docker-compose) ou [Kubernetes](#deployment-templates-and-examples) para permitir reconfiguração incremental e atualizações de imagem.

> Certifique-se de sempre incluir `-e EULA=TRUE`em seus comandos e definições de contêiner, pois a Mojang/Microsoft exige a aceitação do EULA.

> **DO NOT** encaminhamento de porta RCON em 25575 sem primeiro ajuste `RCON_PASSWORD` a um valor seguro. É altamente recomendável usar apenas RCON dentro do contêiner, como com `rcon-cli`.

Por padrão, o contêiner fará o download da versão mais recente do "vanilla" [Minecraft: Java Edition server](https://www.minecraft.net/en-us/download/server) fornecido pela Mojang. O [`VERSION`](#versions)e a [`TYPE`](#server-types) pode ser configurado para criar muitas variações do servidor Minecraft desejado.



## Interacting with the server

[RCON](http://wiki.vg/RCON) está habilitado por padrão, então você pode`exec` no recipiente para
acesse o console do servidor Minecraft:

```
docker exec -i mc rcon-cli
```

Nota: O `-i` é necessário para uso interativo do rcon-cli.

Para executar um comando simples e único, como parar um servidor Minecraft, passe o comando como argumentos para `rcon-cli`, como:

```
docker exec mc rcon-cli stop
```

_O `-i` não é necessário neste caso._

Se o rcon estiver desativado, você pode enviar comandos passando-os como argumentos para o script empacotado `mc-send-to-console`. Por exemplo, um player pode ser aberto no container `minecraft` com: 
...
Para anexar e interagir com o servidor do Minecraft, adicione `-it` ao iniciar o contêiner, como

    `docker run -d -it -p 25565:25565 --name mc lorthe/monga-minecraft`

Com isso você pode anexar e interagir a qualquer momento usando

    `docker attach minecraft`

e depois Control-p Control-q para **detach**.

Para acesso remoto, configure seu daemon Docker para usar um soquete `tcp` (como `-H tcp://0.0.0.0:2375`)
e anexar de outra máquina:

    `docker -H $HOST:2375 attach mc`

A menos que você esteja em uma LAN doméstica/privada, você deve [enable TLS access](https://docs.docker.com/articles/https/).
...
```shell
docker run -d -it --name mc-new -v mc:/data -p 25565:25565 -e EULA=TRUE -e MEMORY=2G lorthe/monga-minecraft
```
...
When using the image `lorthe/monga-minecraft` without a tag, the `latest` image tag is implied from the table below. To use a different version of Java, please use an alternate tag to run your Minecraft server container.
...
For example, to use Java version 8 on any supported architecture:
    ...
    docker run --name mc lorthe/monga-minecraft:java8-multiarch
...
- itzg Helm Chart:
  - [GitHub repo](https://github.com/lorthe/monga-minecraft-charts)
...
The [examples directory](https://github.com/itzg/docker-minecraft-server/tree/master/examples) also provides examples of deploying the [lorthe/monga-minecraft](https://hub.docker.com/r/lorthe/monga-minecraft/) Docker image.
...
The overall version is specified by `VERSION`, [as described in the section above](#versions) and will run the recommended Forge version by default. You can also choose to run a specific Forge version with `FORGE_VERSION`, such as `-e FORGE_VERSION=14.23.5.2854`.
    ...
    docker run -d -v /path/on/host:/data \
        ...
        -p 25565:25565 -e EULA=TRUE --name mc lorthe/monga-minecraft
...
Beware that current [Sponge](https://www.spongepowered.org) `STABLE` versions for Minecraft 1.12 require using [the Java 8 tag](#running-minecraft-server-on-different-java-version):
    ...
    docker run -d -v /path/on/host:/data -e TYPE=SPONGEVANILLA \
        -p 25565:25565 -e EULA=TRUE --name mc lorthe/monga-minecraft:java8-multiarch
...
docker run -d --name mc-ftb -e EULA=TRUE \
  ...
  -p 25565:25565 \
  lorthe/monga-minecraft:java8-multiarch
```
...
> ```
> docker run --env-file=.env lorthe/monga-minecraft
> ```
> 
...
such as:
        ...
        docker run -d -it -e TZ=Europe/London -p 25565:25565 --name mc lorthe/monga-minecraft
...
such as:
        ...
        docker run -d -it -v /etc/timezone:/etc/timezone:ro -p 25565:25565 --name mc lorthe/monga-minecraft
...
To run this image on a RaspberryPi 3 B+, 4, or newer, use any of the image tags [list in the Java version section](#running-minecraft-server-on-different-java-version) that specify `armv7` for the architecture, which includes `lorthe/monga-minecraft:latest`.
